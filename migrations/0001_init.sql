-- ============================================================
-- D1 (SQLite) SCHEMA - HE THONG QUAN LY GIAI TRINH TON DVBH
-- Chuyen doi tu schema.sql (PostgreSQL) sang D1/SQLite:
--   TEXT[]    -> TEXT chua JSON array string (parse o lib/jsonArray.ts)
--   SERIAL    -> INTEGER PRIMARY KEY AUTOINCREMENT
--   UUID      -> TEXT, sinh bang crypto.randomUUID() truoc khi INSERT
--   BOOLEAN   -> INTEGER (0/1)
--   TIMESTAMP -> TEXT (ISO 8601, datetime('now'))
--   NUMERIC   -> REAL
--   users.vai_tro: bo NOT NULL (user moi dang nhap chua co vai tro)
-- ============================================================
...
CREATE TABLE users (
    ...
    ten                 TEXT,
    vai_tro             TEXT CHECK (vai_tro IN (
                            'Admin', 'Viewer', 'QC', 'Giam sat',
                            'TBP DVBH', 'CSKH', 'TN CSKH', 'TBP CSKH'
                        )),
    khu_vuc_phu_trach   TEXT,                 -- JSON array string, vd: ["Ha Noi","Da Nang"]
...
CREATE TABLE settings_ly_do (
    ...
    thuoc_thieu_linh_kien   INTEGER NOT NULL DEFAULT 0,
    created_at              TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at              TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE case_dvbh (
    id                          TEXT PRIMARY KEY,          -- ID tu CRM, co the chua ky tu (vd: CASE-2026-001)
    ...
    tinh                        TEXT,
    quan_huyen                  TEXT,
    thoi_gian_cskh_tiep_nhan    TEXT,
    mo_ta_loi                   TEXT,
    thoi_gian_hen_xu_ly         TEXT,
    nhom_yeu_cau                TEXT,
    loai_yeu_cau                TEXT,
    hang                        TEXT,
    ...
    loai_nganh                  TEXT,
    nhom_san_pham               TEXT,
    thoi_gian_tai_du_lieu_crm   TEXT,

    -- Co nghi ngo vi pham (tra ve san tu CRM, ap dung ratchet 1 chieu)
    loi_120p                    INTEGER NOT NULL DEFAULT 0,
    loi_qua_han_24h             INTEGER NOT NULL DEFAULT 0,
    loi_lo_ke_hoach             INTEGER NOT NULL DEFAULT 0,
    loi_kh_hen_lai              INTEGER NOT NULL DEFAULT 0,

    -- Truong he thong
    ngay_import                 TEXT NOT NULL DEFAULT (datetime('now')),
    ngay_cap_nhat_gan_nhat       TEXT NOT NULL DEFAULT (datetime('now')),
    created_at                   TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at                   TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE INDEX idx_case_loi_120p ON case_dvbh (id) WHERE loi_120p = 1;
CREATE INDEX idx_case_loi_qua_han_24h ON case_dvbh (id) WHERE loi_qua_han_24h = 1;
CREATE INDEX idx_case_loi_lo_ke_hoach ON case_dvbh (id) WHERE loi_lo_ke_hoach = 1;
CREATE INDEX idx_case_loi_kh_hen_lai ON case_dvbh (id) WHERE loi_kh_hen_lai = 1;
...
CREATE TABLE ket_qua_goi (
    ...
    case_id                TEXT NOT NULL REFERENCES case_dvbh(id),
    loai_khao_sat          TEXT NOT NULL,            -- JSON array string, vd: ["Loi 120 phut","Hen qua 24h"]
...
CREATE TABLE vi_pham (
    ...
    case_id              TEXT NOT NULL REFERENCES case_dvbh(id),  -- denormalize
    ...
    chot_bo_cap_2          INTEGER,                  -- NULL = chua QC xet
