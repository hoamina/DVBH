-- Migration 0090: mo rong CHECK(bang) cua settings_audit_log de logAudit() chap nhan
-- "settings_ly_do_cham" (CRUD moi cho danh sach Ly do cham dung khi TN bam "Cho hang"/"Tu choi"
-- trong module Dat mua linh kien, xem routes/settings.ts POST/PATCH /ly-do-cham).
--
-- An toan de recreate-table: KHONG co bang nao khac REFERENCES settings_audit_log(...) - day la
-- bang log 1 chieu, chi no REFERENCES users(email) (nguoi_thay_doi) - xem tien le migration 0036.
CREATE TABLE settings_audit_log_new (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    bang             TEXT NOT NULL CHECK (bang IN ('settings_ly_do', 'settings_ly_do_cham', 'linh_kien', 'settings_phan_loai_tranh_chap', 'settings_ket_qua_xu_ly_tranh_chap')),
